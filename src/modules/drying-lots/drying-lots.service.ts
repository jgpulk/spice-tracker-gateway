...
export class DryingLotsService {
  ...
  async findOne(publicId: string, vendor_id: number) {
    const lot = await this.lotRepo.findOne({
      ...
      relations: ['stock_batches', 'graded_stocks'],
    });
    if (!lot) throw new NotFoundException('Drying lot not found');
    return lot;
  }

  async create(vendor_id: number, lot_name: string, batch_public_ids: string[]) {
    ...
    await this.batchRepo.update(
      { id_stock_batch: In(batchInternalIds) },
      { status: BatchStatus.IN_DRYING, drying_lot_id: lot.id_drying_lot },
    );
    return lot;
  }

  async complete(publicId: string, vendor_id: number, final_dry_weight_kg: number) {
    const lot = await this.findOne(publicId, vendor_id);
    ...
    await this.lotRepo.update(lot.id_drying_lot, {
      final_dry_weight_kg,
