...
export class DryingLotsService {
  ...
  async findOne(id: string, vendor_id: string) {
    const lot = await this.lotRepo.findOne({ where: { id, vendor_id }, relations: ['stock_batches', 'graded_stocks'] });
    if (!lot) throw new NotFoundException('Drying lot not found');
    return lot;
  }

  async create(vendor_id: string, lot_name: string, batch_ids: string[]) {
    ...
    await this.batchRepo.update({ id: In(batch_ids) }, { status: BatchStatus.IN_DRYING, drying_lot_id: lot.id });
    return lot;
  }

  async complete(id: string, vendor_id: string, final_dry_weight_kg: number) {
    const lot = await this.findOne(id, vendor_id);
    const yield_pct = (final_dry_weight_kg / Number(lot.initial_weight_kg)) * 100;
    const batchIds = lot.stock_batches.map((b) => b.id);
    ...
    await this.batchRepo.update({ id: In(batchIds) }, { status: BatchStatus.PROCESSED });
